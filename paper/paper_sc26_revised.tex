...
% ---- Revision marking (CANOPIE-HPC pass) ------------------------------------
% Added/rewritten text is \rev{...} (teal); deletions noted in "% CUT:" comments.
% Accept all: \renewcommand{\rev}[1]{#1}
...
infrastructure---burst cold-start median $7.3$\,s ($n{=}20$), warm pool $\sim$76\,ms, up to $8.4\times$
compression speedup over a $\sim$2\,Mbps link, 94\% GPU utilization\rev{---and report honestly
where each supporting layer helps and where it does not. The rule-based politeness gate defers to
co-tenants under structured or slack load rather than crowding them; the control-theoretic analysis
of feedback admission---including where that politeness is regime-limited---is a companion
contribution}. \rev{All three projects and the reproducibility harness are
released open-source.}
% CUT trailing "---and release all three projects and the harness." -- expanded so
% the abstract's last sentence is the analyzed admission result + the honest
% negative, which is the credibility signal this audience weights most.
\end{abstract}
...
practice/experience paper: not a prototype but self-hosted production infrastructure we run
continuously---the control plane reported here had been live for weeks when we took these
measurements, so the numbers describe the fabric in daily use rather than a staged demo. Its value
is the orchestration design---particularly the politeness controller that makes ``effective and
policy-safe'' mechanical rather than aspirational---and the honest characterization of when each
supporting layer helps.

\rev{\textbf{Fit.} This is a systems paper about operationalizing opportunistic, shared GPU
capacity for AI workloads through \emph{containers and a new orchestration paradigm}: cluster
pods reached as first-class subscribers on the researcher's own message bus, isolated and
policy-bounded by construction. The contribution is not a new scheduler or a new quantizer, but a
\emph{policy-aware, container-native bursting fabric}---an orchestration design that turns
multi-tenant Kubernetes GPUs into event-loop-local capacity while respecting the host's isolation
and fair-use policy, with reproducible measurements of cold start, warm-pool latency, compressed
transport, leaf-node federation behavior, and GPU utilization.}
% CUT: "Why SC. ..." -- reframed for CANOPIE-HPC (Containers and New Orchestration
% Paradigms for Isolated Environments in HPC): the paper's containerization,
% orchestration, and isolation/policy angles are foregrounded, which is exactly
% that workshop's scope.
...
``ignored'' range), but every running GPU pod must sustain utilization \emph{above} the policy
floor (${\ge}$40\% GPU), no more than $K{=}4$ pods run concurrently per user, Jobs must run to
completion and exit cleanly (no \texttt{sleep}-camping), and requests should match usage. ``Effective and policy-safe'' is therefore a container-orchestration design
target, not a slogan.
...
\begin{table}[t]\centering\caption{Politeness controller defaults. The self-limits (50/20) are the
software's conservative caps for a permissive host; on NRP the host pod cap ($K{=}4$) binds
first, so runs here stayed at ${\le}4$ concurrent.}\label{tab:cfg}
...
Ephemeral CPU Jobs (ignored range, one at a time, auto-cleaned) cold-start in median $7.3$\,s
(IQR $6.5$--$8.0$, range $4.2$--$9.8$, $n{=}20$), of which the K8s schedule$\to$container-start is
$\sim$1--3\,s (median $2$\,s). The pooled median mixes two node classes, so we report
each: the sixteen runs on the primary node (\texttt{gp-argo}) have median $7.5$\,s,
and the four that landed on a warmer/closer node (\texttt{rci-nrp-gpu-04}) median
$4.9$\,s---the split, not a single headline number, is the honest summary. A
persistent pool of $N$ queue-group workers driven from the hub (Table~\ref{tab:t3},
Fig.~\ref{fig:t3}) serves at $\sim$76\,ms per task---removing the $\sim$100$\times$ cold-start
penalty. Throughput is flat across $1$--$4$ workers ($\sim$700--740/s), and honestly so: for
trivial I/O-bound tasks the ceiling is concurrency\,$\div$\,RTT (Little's law, $64/0.076\approx
840$/s), not worker count---one pod already saturates the WAN link. Worker-count scaling is
expected to matter for \emph{compute-bound} tasks. These Tier-3 numbers are measured over the
\emph{same continuously-running deployment} we use ourselves---the in-cluster NATS leaf had been up
21 days and the controller 10 at the time of measurement---so they report the fabric in production
use, not a staged demo. Finally, batch-probe sized a BERT-base-scale encoder to batch 204 on a
...
1 & 697/s & 80\,ms & 128\,ms \\
2 & 710/s & 77\,ms & 116\,ms \\
4 & 737/s & 76\,ms & 104\,ms \\\bottomrule
\end{tabular}
\end{table}
...
\subsection{Admission control (companion)}\label{sec:eval-admission}
The fabric's politeness rests on the rule-based gate of \S\ref{sec:bursting}
deferring under load rather than grabbing the cap; the utilization and warm-pool
measurements above already exercise it. The \emph{control-theoretic} treatment of
feedback admission---the goodput/politeness frontier under real, time-varying
contention, an age-of-information limit on how much feedback can possibly help, and
a regime-adaptive controller---is a distinct contribution developed and evaluated in
a companion paper~\cite{companion}, and we deliberately do not reproduce it here so
the two results stay disjoint. \rev{One qualifier from that analysis carries over:
politeness is \emph{regime-dependent}. Feedback admission spares the neighbour under
structured or slack load, but under \emph{memoryless} contention that outpaces the
detection delay it can crowd the co-tenant as much as a naive policy---so ``polite''
is a property of the structured/slack regime this deployment operates in, not an
unconditional guarantee. The rule-based gate here inherits the same limit.} The
thermal micro-tier of \S\ref{sec:bp} ran live
during these bursts, throttling $>\!90\%$ of GPU epochs and, on the shared node,
finding the co-tenant thermal baseline ($\sim$$77^\circ$C) a floor self-throttling
cannot undercut---politeness extends even to heat.
...
reaches the GPU-bound ceiling---$9.4$k\,docs/s at four workers, scaling to $14.9$k at eight
\emph{on this single GV100}---competitive with the in-process engines (Ray $9.8$k, ProcessPool
$9.0$k) and far above Dask ($1.1$k); its pure-fabric ceiling (a near-empty task) is $8.3$k\,req/s.
Fig.~\ref{fig:embed-scaling} is a \emph{different, larger} configuration---the
compute-bound scaling run on \emph{two} GV100s---and reaches ${\sim}44$k\,docs/s at
eight workers. The $14.9$k here (one GPU, eight workers) and that ${\sim}44$k (two
GPUs, eight workers) are separate experiments and should not be read as one scaling
curve. (A first-draft single-client
...
($13.9$\,ms). The Go client contributes to this figure---a Python-client pool sits nearer the
interpreter-bound engines---so we do not lean on the latency win. Indeed, raw dispatch is not the
contribution: Ray, Dask, Parsl and Globus Compute/funcX are
...
nats-bursting). ``by design'' = not run on this testbed but placed from documented
architecture: Parsl's HTEx interposes a process/worker executor comparable to
\texttt{ProcessPool}, and funcX/Globus Compute requires a hosted, authenticated
endpoint (so it cannot run same-node); their polite/WAN/bus entries follow from that
architecture, not a measurement. polite = host fair-use budget; WAN = inbound-restricted leaf federation;
bus = burst is an event-loop publish. $^{\dagger}$Go client; a Python-client pool
sits nearer the interpreter-bound engines, so we do not lean on the dispatch win
(\S\ref{sec:eval-frameworks}).}\label{tab:frameworks}
\small
...
\textbf{nats-bursting} & \textbf{0.9\,ms}$^{\dagger}$ & \textbf{9400} & \textbf{yes} & \textbf{yes} & \textbf{yes} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Cold-start realism (Tier 3):} the $n{=}20$ median mixes two nodes---image-pull state and
\texttt{kubectl wait} detection inflate the total, and the schedule$\to$run breakdown ($\sim$2\,s
median) isolates the cluster-side cost; warm-pool and a real NATS-join are
measured, while a cold image pull and a GPU-image \emph{throughput} run remain (the
end-to-end GPU submission path---submit\,$\to$\,node-targeted GPU worker\,$\to$\,federated
result---is itself verified in the companion~\cite{companion}, just not benchmarked at
scale here). \textbf{Scaling:} the plateau is
explained by Little's law, not presented as pod-count scaling; a compute-bound load is the named
next run. \textbf{Politeness gate.} \rev{The rule-based gate of \S\ref{sec:bursting} is evaluated here only
through its effect on utilization and warm-pool behaviour; its control-theoretic characterization
under real contention---the goodput/politeness frontier, the feedback-usefulness limit, and the
regime-adaptive controller---is the companion paper's~\cite{companion} subject, kept out of this
paper so the two contributions stay disjoint.}
% CUT: original one-line "Contention realism" note. The result is unchanged; the
% threat is reframed from a bare confession into (i) what the controlled design
% does prove (causal attribution), (ii) what it does not (scale/heterogeneity),
% and (iii) the concrete experiment that would close it -- which is what an HPC
% reviewer who runs real clusters wants to see acknowledged. \textbf{Portability:}
...
\textbf{When to use which shape.} An ephemeral Job costs a $\sim$7\,s cold start but holds no
resources between bursts---right for occasional or embarrassingly parallel work. A persistent pool
pays that once and serves at $\sim$76\,ms but occupies its slots---right for interactive or
...
throughput; Job cold-start; warm-pool latency/throughput; GPU utilization.
...
% Admission-control harness intentionally omitted: it belongs to the companion
% paper~\cite{companion}, not this artifact, so the two submissions stay disjoint.
...
round-trip speedup at 256\,KB over the $\sim$2\,Mbps hop; Tier~3 median cold-start $\sim$7.3\,s ($n{=}20$),
warm-pool $\sim$76\,ms, throughput $\sim$740/s (the Little's-law bound); and GPU
right-sizing $\approx$94\% utilization on a GV100. Absolute latencies, cold-start, and utilization
depend on the cluster, WAN path, and GPU model; the qualitative results (compression win grows with
payload, warm pool removes the cold-start penalty) are hardware-independent.
