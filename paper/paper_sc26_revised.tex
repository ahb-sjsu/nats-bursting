...
co-tenants under load rather than crowding them; the control-theoretic analysis of feedback
admission is a companion contribution}. \rev{All three projects and the reproducibility harness are
...
$\sim$1--3\,s (median $2$\,s); the four fastest runs landed on a warmer/closer node ($\sim$4--5\,s),
the rest on one $\sim$7\,s node. A
...
the two results stay disjoint. The thermal micro-tier of \S\ref{sec:bp} ran live
...
(The two-GV100 scaling run of Fig.~\ref{fig:embed-scaling} is a separate, larger configuration and
reaches ${\sim}44$k at eight workers; the two numbers are different setups, not the same
experiment.) (A first-draft single-client
