...
$(w_t-a_t)^{+}=\sum_{u\ge1}\mathbf1[w_t\ge u](1-x^u_t)$. The age-$D$ optimum is
the delayed-capacity window $w_t=a_{t-D}$, and its total goodput advantage over
the best comonotone fixed budget at matched surplus is
\begin{equation}\label{eq:adv-multi}
\Delta(D)=\sum_{u=1}^{K}\pi_u\,r_u(D),\qquad r_u(D)=\mathrm{corr}\big(x^u_t,x^u_{t-D}\big),
\end{equation}
which for a single symmetric contended unit with $\pi_u=\tfrac12$ recovers
$\Delta=\tfrac12 r(D)$.
\end{proposition}
\begin{proof}
The two identities are the layer-cake expansions of $\min$ and $(\cdot)^+$.
Setting $w_t=a_{t-D}$ makes $\mathbf1[w_t\ge u]=x^u_{t-D}$, the
Thm.~\ref{thm:limit} threshold policy applied independently to each layer, and the
...
with $\Pr[x^u{=}1]=\pi_u$ the same linear program as Thm.~\ref{thm:limit} gives
per-unit advantage $\mathrm{Cov}(x^u_t,x^u_{t-D})/(1-\pi_u)=\pi_u r_u(D)$ over the
fixed policy matching its marginal, and additivity of goodput and surplus sums
these to \eqref{eq:adv-multi}.
\end{proof}
...
advantage is the per-unit sum \eqref{eq:adv-multi}. It reduces to
Algorithm~\ref{alg:adaptive} when only one layer fluctuates.
