...
\date{Working draft --- v0.9 (selection-aware bootstrap executed: seismic certificate survives at 0.042 nats, three routes agree)}
...
floor, and finds one clearly material history gain; it illustrates the
...
$\varepsilon_{\mathrm{un}}$ & $U_2$ & paired boot.@ (fixed $D^*$) & verdict \\
\midrule
...
the practical floor with a paired bound at the selection floor. VAD's degenerate $[.000,.000]$ interval is genuine: at its $D^*{=}1$
the $k{=}6$ policy and the exact single-sample policy reduce to the same
majority rule on every fold, so the paired difference is identically
zero. The i.i.d.\ null's \emph{positive} paired interval is the
...
dependence- \emph{and} selection-aware: with the lag selection repeated
inside every block-bootstrap replicate, the one-sided bound is
$+0.1453$, giving
$I(X_t;\text{history}\mid X_{t-D^*})\ge 2(0.1453)^2\approx0.042$
nats --- a certificate in the defensible sense. Two further routes
corroborate to within a few percent: the fixed-$D^*$ moving-block
interval ($[+.146,+.168]$, $\approx0.042$) and the fitted-Markov
surrogate calibration ($2(0.158-0.014)^2\approx0.041$) --- alongside
the total-information witness $I(X_t;\text{window})\ge0.176$ nats. History knows what the last
