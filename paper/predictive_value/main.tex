...
\date{Working draft --- v0.5 (review round 2: incremental-information theorem, contact-order law, statistical renaming)}
...
the weak-correlation exponent (Theorem~
ef{thm:contact}).
...
estimates sample the pipeline's estimation error \emph{under the fitted
Markov model}; the $95$th percentile $\varepsilon_{\mathrm{dom}}$ is
therefore a \emph{model-calibrated} (not distribution-free) bound, and
every one-sided interval below inherits that qualification --- a
dependence-aware alternative (moving-block bootstrap on held-out paired
loss differences) is the registered upgrade path for a submission build.
...
express. Bold: the only single-lag-envelope excess clearing the
model-calibrated noise bound --- per Theorem~
ef{thm:incr} a signature of
higher-order dependence, not a violation. Pooled pair counts range from $2.0$k (protons) to $21$k
(azimuth) per lag.}
\label{tab:battery}
\end{table}
...
registered practical threshold; the remaining series show no detected
excess, with equivalence established only where the model-calibrated upper
bound falls below the margin (Markov reference, azimuth). Where noise is
wide (short whale exchanges, sparse protons), the verdict stays at ``no
excess detected,'' honestly labelled.
