...
\date{Working draft --- v0.8 (review round 4: consolidated on battery v5; sign-branch thresholds; certification language scoped)}
...
lower bounds are claimed only above that floor, and repeating the lag
selection inside each bootstrap replicate is the registered refinement
for the camera-ready analysis. Validity checks pass:
...
$\varepsilon_{\mathrm{un}}$ & $U_2$ & paired 95\% CI & verdict \\
\midrule
Two-state Markov & $.470$ & $+.000$ & $.78$ & $.006$ & $.000$ & $.000$ & $[-.003,+.014]$ & equivalent \\
i.i.d.\ null & $.497$ & $+.012$ & $.023$ & $.011$ & $-.001$ & $.011$ & $[+.005,+.020]$ & selection floor \\
LOCATA azimuth & $.494$ & $+.008$ & $.063$ & $.009$ & $.020$ & $.027$ & $[-.003,-.000]$ & equivalent \\
LOCATA VAD & $.636$ & $-.000$ & $.82$ & $.052$ & $.000$ & $.000$ & $[.000,.000]$ & equivalent \\
Sperm-whale codas & $.483$ & $+.046$ & $1.0$ & $.117$ & $-.063$ & $-.016$ & $[-.055,-.018]$ & equivalent \\
GOES protons & $.489$ & $+.029$ & $.11$ & $.033$ & $.003$ & $.032$ & $[+.002,+.051]$ & not established \\
GOES X-ray flux & $.421$ & $+.028$ & $<.004$ & $.008$ & $.001$ & $.028$ & $[+.011,+.037]$ & detected, below floor \\
GOES magnetometer & $.456$ & $+.033$ & $<.004$ & $.010$ & $.001$ & $.033$ & $[+.022,+.048]$ & detected (exploratory) \\
Seismic ANMO & $.508$ & $+.158$ & $<.004$ & $.014$ & $.000$ & $.158$ & $\mathbf{[+.146,+.168]}$ & history adds \\
\bottomrule
\end{tabular}
...
the practical floor with a paired bound at the selection floor. The
i.i.d.\ null's \emph{positive} paired interval is the empirical
lag-selection bias (its upper end, $\approx0.02$, is the claim floor for
...
the Pinsker side of unity as population theory requires).
...
bound on conditional predictive information. The \emph{primary} result
is the dependence-aware one: the moving-block interval
$[+.146,+.168]$ gives $I(X_t;\text{history}\mid X_{t-D})\ge
2(0.146)^2\approx0.043$ nats, with the stated caveat that $D^*$ was
selected before bootstrapping (the i.i.d.-calibrated selection floor,
$0.02$, is dwarfed sevenfold). The fitted-Markov surrogate route
corroborates with $2(0.158-0.014)^2\approx0.041$ nats --- two
complementary calibration routes, agreeing to within a few percent ---
alongside the total-information witness
$I(X_t;\text{window})\ge0.176$ nats. History knows what the last
sample does not; that is what ``aftershock clustering'' means in
information units. Magnetometer's paired bound ($+.022$) sits at the
selection floor and is reported as exploratory. The instrument reads in
...
dead zone is its polyhedral counterpart. The information envelope itself is assembled from
...
Russo and Van Roy \cite{russo16}. To our knowledge the bounded-objective envelope \eqref{eq:mi} has not
been stated as such; we accordingly claim assembly and anchoring, not
the inequality. What we have not found
