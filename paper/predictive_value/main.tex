...
\date{Working draft --- v0.4 (battery v4 results integrated: dead zone observed, VAD reversal, one certified witness)}
...
theorem into confidence-certified lower bounds on history information where
the processes are non-Markov.
\end{abstract}
...
\cite{radnerstiglitz84}: for asymmetric states under a polyhedral
...
theorem as a certified witness of history information on non-Markov
series, always through a one-sided lower confidence bound:
$I(X_t;\text{history})\ge 2[(\widehat\Delta-\varepsilon)^{+}]^2$
(\S\ref{sec:battery}).
...
\subsection{Smooth objectives: quadratic response, and the trichotomy}
\label{sec:smooth}
...
\begin{remark}[The trichotomy: local decision geometry sets the
correlation exponent]\label{rem:trichotomy}
The cases exhaust the local behaviours of a convex $\psi$ at the prior, and
each fixes how weak dependence buys value: \emph{prior on a kink}
...
Radner--Stiglitz nonconcavity of the value of information
\cite{radnerstiglitz84}, whose smooth setting is our third case (zero
...
identical pipeline, with $M=299$ surrogates. Significance uses the exact
Monte Carlo form $p=(1+\#\{T_j\ge T_{\mathrm{obs}}\})/(M+1)$ (smallest
reportable value $1/300\approx0.0033$; entries at that floor are written
$p<.004$, never $.00$), together with the companion study's practical floor
of $0.03$ on the effect. Because each surrogate is Markov, its \emph{true}
history gain is zero and its own exact $\widehat V_1$ is computable from
its lag table, so the $M$ surrogate gain estimates form a genuine sample of
the pipeline's \emph{estimation error under matched dependence}; its
$95$th percentile $\varepsilon_{\mathrm{dom}}$ calibrates every one-sided
bound below. \emph{Non-rejection is not exactness}: where the test does
not fire, we report the one-sided upper confidence bound
$U=\text{gain}_{\max}+\varepsilon_{\mathrm{dom}}$ and claim ``no
detectable excess (equivalent within the margin)'' only when $U<0.03$.

\textbf{The envelope measurement.} For each domain and lag, the plug-in
mutual information $\widehat I(X_t;X_{t-D})$ from pooled pair counts
($2{\times}2$; plug-in bias $\le 3/(2N)$ nats, negligible at the pooled
sample sizes reported) and the envelope $\sqrt{\widehat I/2}$ of
Theorem~\ref{thm:mi}. Two registered uses. \emph{Forward} (envelope
check): $\widehat\Delta_{\mathrm{hist}}(D)\le\sqrt{\widehat I(D)/2}
+\varepsilon_{\mathrm{dom}}$ --- an apparent violation smaller than the
matched-dependence noise is estimator noise, not physics. \emph{Backward}
(certified witness): a policy value is converted into an information lower
bound only through its one-sided \emph{lower} confidence bound,
\[
I(X_t;\text{window})\;\ge\;2\,\bigl[(\widehat\Delta_{\mathrm{hist}}
-\varepsilon_{\mathrm{dom}})^{+}\bigr]^{2},
\]
...
$U$ & env.\ excess & verdict \\
\midrule
Two-state Markov (ref) & $.470$ & $+.000$ & $.78$ & $.006$ & $.007$ & $-.007$ & equivalent \\
i.i.d.\ null (control) & $.497$ & $+.012$ & $.023$ & $.011$ & $.023$ & $+.009$ & below floor \\
LOCATA azimuth         & $.494$ & $+.008$ & $.063$ & $.009$ & $.016$ & $-.026$ & equivalent \\
LOCATA VAD             & $.636$ & $-.000$ & $.82$  & $.052$ & $.052$ & $-.027$ & no excess detected$^{\dagger}$ \\
Sperm-whale codas      & $.483$ & $+.046$ & $1.0$  & $.117$ & $.164$ & $+.027$ & no excess detected$^{\dagger}$ \\
GOES protons           & $.489$ & $+.029$ & $.11$  & $.033$ & $.063$ & $+.021$ & no excess detected$^{\dagger}$ \\
GOES X-ray flux        & $.421$ & $+.028$ & $<.004$ & $.008$ & $.036$ & $-.008$ & detected, below floor \\
GOES magnetometer      & $.456$ & $+.033$ & $<.004$ & $.010$ & $.043$ & $-.004$ & history adds (at floor) \\
Seismic ANMO           & $.508$ & $+.158$ & $<.004$ & $.014$ & $.171$ & $\mathbf{+.148}$ & history adds (certified) \\
\bottomrule
\\end{tabular}
\caption{Battery v4 (corrected protocol; \texttt{battery\_v4.json}).
``Equivalent'' $=$ $U<0.03$; $^{\dagger}$``no excess detected'' $=$ test
does not fire but $U\ge 0.03$, so equivalence is \emph{not} established
(noise-limited) --- a distinction the uncorrected protocol could not
express. Bold: the only envelope excess that clears the matched-dependence
noise bound. Pooled pair counts range from $2.0$k (protons) to $21$k
(azimuth) per lag.}
\label{tab:battery}
\end{table}
...
\textbf{Single-sample sufficiency is the rule.} On eight of nine series the
delayed single sample, evaluated exactly (majority baseline, dead zones
included), captures all detectable feedback value at the battery's
resolution --- with the equivalence bound $U<0.03$ certified for the
balanced, well-sampled series (Markov reference, azimuth) and the weaker
``no excess detected'' verdict, honestly labelled, where noise is wide
(short whale exchanges, sparse protons).

\textbf{The dead zone is observed.} The imbalanced GOES series realize
Theorem~\ref{thm:hinge} in the wild: X-ray ($\hat\pi=.421$, kink at
$\tfrac12$, threshold $C^*\approx0.033$) has $\widehat V_1$ \emph{exactly
zero} across its moderate-covariance lags --- the majority policy is
unbeatable by any single delayed sample --- while its plug-in information
remains positive; its envelope-to-value ratio diverges ($\gg10^6$) not as
an artifact but as the theorem's geometry: \emph{information without value},
Proposition~\ref{prop:noneq} measured on a physical process. Freshness
metrics, and information metrics alike, cannot see this; the Bayes envelope
does.

\textbf{The certified witness stands --- once.} Seismic regime is the one
series where the history policy's excess over the single-lag envelope
clears the matched-dependence noise bound ($+0.148$ vs.\
$\varepsilon=.014$), and its confidence-certified window-information bound
is $I(X_t;\text{window})\ge 0.176$ nats --- aftershock clustering carrying
an order of magnitude more predictive information than the last sample
alone ($\widehat I_1\approx0.002$ at the certifying lag). Magnetometer
shows a small certified gain at the practical floor. The instrument reads
in both directions, and under the corrected statistics its two directions
are: equivalence, most places; certified history information, exactly where
the physics says it should be.
...
Radner--Stiglitz nonconcavity \cite{radnerstiglitz84} anticipates the dead
zone in smooth form. The information envelope itself is assembled from
textbook inequalities --- kindred moves appear wherever bounded value must
be controlled by divergence, e.g.\ the information-ratio regret analyses of
Russo and Van Roy \cite{russo16}. What we have not found
...
been quantified directly \cite{soleymani22}. Our claim is accordingly not
...
\bibitem{soleymani22}
T.~Soleymani, J.~S.~Baras, K.~H.~Johansson. Value of information in
