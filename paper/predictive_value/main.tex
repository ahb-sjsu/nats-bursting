...
\date{Working draft --- v0.6 (review round 3: policy-class scoping, two-tail calibration, conditional-certificate column, ref repairs)}
...
bounds; a dependence-aware alternative (moving-block bootstrap on held-out
paired loss differences, with both tails resolved) is the registered
upgrade path for a submission build.
...
Theorem~\ref{thm:incr}, seismic's history gain over the exact single-sample
value converts into a model-calibrated certificate of \emph{conditional}
predictive information:
$I(X_t;\text{history}\mid X_{t-D})\ge 2(0.158-0.014)^2\approx
0.041$ nats --- history knows what the last sample does not, which is what
``aftershock clustering'' means in information units --- alongside the
total-information witness $I(X_t;\text{window})\ge0.176$ nats. The
