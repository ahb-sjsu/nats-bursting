...
\date{Working draft --- v0.7 (battery v5: both tails + dependence-aware certificates; lit-check verdicts in text)}
...
process, together with its \emph{incremental} refinement --- the value of
...
sign-dependent dead zones; for smooth objectives it is exactly quadratic
(squared error: $\Delta=C^2/\pi(1-\pi)$); the companion's linear law is the
prior-on-kink boundary case. A nine-series battery (acoustics,
bioacoustics, space weather, seismology, plus synthetic controls) finds no
detectable history excess (equivalence-bounded within a registered margin)
on the series where lag-1-matched surrogates predict none, observes the
envelope's predicted $O(r^2)$ tightness on real data, and inverts the
incremental theorem into model-calibrated lower bounds on conditional
history information where the processes are non-Markov.
\end{abstract}
...
thresholds --- linear response beyond, identically zero within; for smooth
objectives (e.g.\ squared-error prediction, where exactly
$\Delta=C(D)^2/\pi(1-\pi)$) the response is \emph{quadratic}; and a prior
sitting on a kink --- the companion's symmetric case --- is the unique
configuration with threshold-free linear response
...
battery across four physical domains bounds any undetected history excess
within a registered equivalence margin where surrogates predict none,
observes the envelope's tightness, and --- read backwards --- uses the
incremental theorem as a certified witness of \emph{conditional} history
information on non-Markov series, always through a one-sided
model-calibrated lower bound:
...
\emph{Mixing (lower bound).} The window $[t-m,\,t-D]$ has length $K$, hence
contains exactly one preview slot $s\equiv\Theta$; that preview is in the
...
$\pi=\tfrac12$ or the kink set is symmetric about $\pi$, the two branches
differ --- positive and negative correlation of the same magnitude can have
different value, one of them zero.
\end{theorem}
...
Theorem~\ref{thm:contact} subsumes the named regimes --- $q=1$ (kink):
linear response; locally affine: dead zone; $q=2$ with
$c_\pm=\tfrac12\psi''(\pi)>0$: quadratic --- and admits fractional contact
orders $q\in(1,2)$, realizable by valid suprema of affine rewards, with
correspondingly fractional correlation exponents. Each case fixes how weak
...
puts the prior on the decision kink, the unique threshold-free polyhedral
configuration. The dead zone is the sharp polyhedral form of the
Radner--Stiglitz nonconcavity of the value of information
...
$U=\hat g+\varepsilon_{\mathrm{under}}$; the two coincide only under a
symmetry condition. The present run recorded the single upper-tail
quantile $\varepsilon_{\mathrm{dom}}=\varepsilon_{\mathrm{over}}$ and uses
it on both sides --- exact for every lower certificate, an approximation
for the equivalence labels, which therefore carry it as a stated
limitation. Both are \emph{model-calibrated} (not distribution-free)
bounds. The registered upgrade path has since been executed (battery v5):
(a) \emph{both tails resolved} --- $arepsilon_{\mathrm{under}}$ proves
near zero or negative on most series (the error distribution sits above
zero), so the v4 equivalence labels were conservative, and under the
two-tail bound $U_2$ two further series (VAD, whale codas) become
class-equivalent while protons remains just above the margin
($U_2=.032$); (b) a \emph{dependence-aware} moving-block bootstrap on
held-out paired loss differences --- the registered $k{=}6$ policy against
the pooled-\emph{exact} single-sample policy (in-sample fit favouring the
single-sample arm, hence conservative for the certificate), blocks
resampled within runs at the selected lag $D^*$. Because $D^*$ is an
argmax over the lag grid, the i.i.d.\ null's paired interval calibrates
the \emph{selection} bias the bootstrap does not cover (its 95th
percentile $pprox0.02$ at the selected lag); certificates are claimed
only where the paired lower bound clears that floor. Validity checks pass:
...
report the model-calibrated upper bound
$U=\text{gain}_{\max}+\varepsilon_{\mathrm{dom}}$ and claim ``no
detectable excess (equivalent within the margin)'' only when $U<0.03$.
...
plug-in bias $\le 3/(2N)$ nats, negligible at the pooled sample sizes
...
$\widehat V_1$ is the empirical increment $\Delta_W-\Delta_Z$, and its
model-calibrated lower bound certifies \emph{conditional} predictive
information:
\[
I(X_t;\,\text{history}\mid X_{t-D})\;\ge\;
2\,\bigl[(\text{gain}_{\max}-\varepsilon_{\mathrm{dom}})^{+}\bigr]^{2},
\]
...
-\varepsilon_{\mathrm{dom}})^{+}]^{2}$ is retained separately.

\subsection{Results}

Table~\ref{tab:battery} reports the corrected battery (v4): per domain, the
class proportion $\hat\pi$; the maximum history gain over the exact
single-sample Bayes value $\widehat V_1$; the exact Monte Carlo $p$
($M{=}299$; floor $1/300$, written $<.004$); the matched-dependence noise
bound $\varepsilon_{\mathrm{dom}}$; the equivalence bound
$U=\text{gain}_{\max}+\varepsilon_{\mathrm{dom}}$ (margin $0.03$); the
maximum excess of the history policy over the single-lag envelope, with
certification only when it clears $\varepsilon_{\mathrm{dom}}$; and the
certified window-information lower bound
$2[(\widehat\Delta_{\mathrm{hist}}-\varepsilon_{\mathrm{dom}})^+]^2$
where applicable.

\begin{table}[h]\centering\small
\begin{tabular}{lrrrrrrl}
\toprule
Domain & $\hat\pi$ & gain$_{\max}$ & $p$ & $\varepsilon_{\mathrm{dom}}$ &
$U$ & $I^{\mathrm{cond}}_{\mathrm{LB}}$ & verdict \\
\midrule
Two-state Markov (ref) & $.470$ & $+.000$ & $.78$ & $.006$ & $.007$ & $0$ & equivalent$^{*}$ \\
i.i.d.\ null (control) & $.497$ & $+.012$ & $.023$ & $.011$ & $.023$ & $.000$ & below floor \\
LOCATA azimuth         & $.494$ & $+.008$ & $.063$ & $.009$ & $.016$ & $0$ & equivalent$^{*}$ \\
LOCATA VAD             & $.636$ & $-.000$ & $.82$  & $.052$ & $.052$ & $0$ & none detected$^{\dagger}$ \\
Sperm-whale codas      & $.483$ & $+.046$ & $1.0$  & $.117$ & $.164$ & $0$ & none detected$^{\dagger}$ \\
GOES protons           & $.489$ & $+.029$ & $.11$  & $.033$ & $.063$ & $0$ & none detected$^{\dagger}$ \\
GOES X-ray flux        & $.421$ & $+.028$ & $<.004$ & $.008$ & $.036$ & $.001$ & detected, below floor \\
GOES magnetometer      & $.456$ & $+.033$ & $<.004$ & $.010$ & $.043$ & $.001$ & history adds (at floor) \\
Seismic ANMO           & $.508$ & $+.158$ & $<.004$ & $.014$ & $.171$ & $\mathbf{.041}$ & history adds (certified) \\
\bottomrule
\end{tabular}
\caption{Battery v4 (corrected protocol; \texttt{battery\_v4.json}).
$I^{\mathrm{cond}}_{\mathrm{LB}}=2[(\hat g-\varepsilon_{\mathrm{dom}})^+]^2$
nats: the model-calibrated conditional-information certificate of
Theorem~\ref{thm:incr} --- the scientifically meaningful quantity; bold
where material. Verdicts are statements about the \emph{registered
six-lag lookup class}: $^{*}$``equivalent'' $=$ $U<0.03$ under the
symmetric-tail approximation stated in the protocol;
$^{\dagger}$``none detected'' $=$ test does not fire but $U\ge 0.03$, so
class equivalence is \emph{not} established (noise-limited). Pooled pair
counts range from $2.0$k (protons) to $21$k (azimuth) per lag.}
\label{tab:battery}
\end{table}

Four readings, in increasing order of consequence.

\textbf{The corrections bite exactly where the review predicted.} Under the
chance baseline (v3), four series appeared to carry history value beyond
the single sample (gains $0.100$--$0.367$, all ``$p=.00$''). Under the
majority baseline and exact $\widehat V_1$: LOCATA VAD's gain
\emph{vanishes} ($+0.100\to-0.000$, $p=.82$; $\hat\pi=.636$ --- the entire
effect was the majority-class head start), GOES X-ray shrinks
$0.185\to0.028$ (below the practical floor; $\hat\pi=.421$), magnetometer
$0.147\to0.033$ (at the floor), and only seismic survives at strength
($0.367\to0.158$). Imbalance, not history, carried most of the apparent
excess --- and the sub-unity envelope ratios of v3 disappear with it (all
balanced-domain tightness ratios now sit at $1.11$--$1.35$, on the Pinsker
side of unity, exactly as population theory requires).

\textbf{Only seismic exhibits a clearly material history gain.} X-ray and
magnetometer show smaller statistically detected gains at or below the
registered practical threshold; on the remaining series \emph{no
additional value was detected by the registered six-lag lookup policy},
with class equivalence established only where the model-calibrated upper
bound falls below the margin (Markov reference, azimuth). Where noise is
wide (short whale exchanges, sparse protons), the verdict stays at ``none
detected,'' honestly labelled --- and none of these negative findings
...
($\hat\pi=.421$, kink at $\tfrac12$, threshold $C^*\approx0.033$) has
$\widehat V_1$ exactly zero across its moderate-covariance lags --- the
majority policy is unbeatable by any single delayed sample --- while its
plug-in information remains positive: \emph{information without value},
the phenomenon of Proposition~\ref{prop:noneq}, on a physical series. (The
envelope-to-value ratio is undefined where $\widehat V_1=0$ and is not
reported there; population uncertainty for the dead-zone boundary itself
is future work, so we label this an \emph{empirical} dead zone.)

\textbf{The conditional certificate stands --- once at strength.} By
Theorem~\ref{thm:incr}, seismic's history gain converts into a certificate of
\emph{conditional} predictive information by two independent calibration
routes that agree to within 3\%: model-calibrated,
$2(0.158-0.014)^2\approx0.041$ nats; and dependence-aware
(moving-block bootstrap, conservative exact-arm pairing, one-sided 95\%),
$2(0.1456)^2\approx0.042$ nats --- history knows what the last sample
does not, which is what ``aftershock clustering'' means in information
units --- alongside the total-information witness
$I(X_t;\text{window})\ge0.176$ nats. Magnetometer's paired bound
($0.022$) clears the selection floor marginally; no other series does. The
seismic history value also exceeds the \emph{single-lag} envelope
($+0.148$ vs.\ $\varepsilon=.014$); under Theorem~\ref{thm:incr} this is
not a violation of anything but the expected signature of higher-order
dependence, and it co-occurs with the certified conditional information
exactly as the theory requires. Magnetometer shows a small detected gain
at the practical floor (conditional certificate $\approx0.001$ nats ---
reported, not emphasized). The instrument reads in both directions:
equivalence or bounded excess on most series; certified conditional
history information on the series where it is found.
...
study the value function as a support function, characterize when
information has positive value, and give local estimates near
uninformative experiments. Our Lipschitz lemma instantiates their
payoffs--beliefs duality; the hinge and dead-zone analysis of
\S\ref{sec:multistate} is the exact delayed-binary specialization of that
convex-analytic programme; the comparison-of-experiments order behind
Lemma~\ref{lem:blackwell} is Blackwell's \cite{blackwell53}; and the
Radner--Stiglitz nonconcavity \cite{radnerstiglitz84,chadeschlee02} anticipates the dead
zone in smooth form. The information envelope itself is assembled from
textbook inequalities, and its closest classical relatives are the
Fano-type families bounding Bayes \emph{error} by information; kindred
moves appear wherever bounded value must be controlled by divergence,
e.g.\ the information-ratio regret analyses of
Russo and Van Roy \cite{russo16}. A targeted search (2026-07-22) found the
adjacent families but not the bounded-objective envelope
\eqref{eq:mi} stated as such; we accordingly claim assembly and
anchoring, not the inequality. What we have not found
...
and history gains beyond the single-sample value certify conditional
predictive information in nats. The instrument reads in both directions; that, more
