...
\date{Working draft --- v0.3 (major revision: polyhedral restriction + trichotomy, sign branches, preview repair, corrected statistics; battery v4 executing)}
...
\emph{[Interim: the table below is the v3 run --- chance baseline, $M=60$,
point-estimate witnesses --- retained as the repository record while the v4
run under the corrected protocol above ($M=299$, majority baseline, exact
$\widehat V_1$, equivalence and certificate bounds) executes; the table and
prose will be replaced by v4. Two v3 artifacts are already explained by the
correction: env$/$law ratios below one are impossible in population for
\emph{balanced} pairs ($\sqrt{I/2}\ge|r|/2$ is Pinsker), so the sub-unity
entries flag exactly the imbalanced series the chance baseline mishandles;
and the ``$p=.00$'' entries are $p<.017$ at $M=60$.]}

Table~\ref{tab:battery} reports, per domain: the maximum history gain
with its surrogate $p$-value; the per-domain noise bound
$\varepsilon_{\mathrm{dom}}$; the maximum
...
\midrule
Two-state Markov (ref)   & $0.030$ & $.38$ & $0.069$ & $+0.023$ & $1.001$ & no detectable excess \\
i.i.d.\ null (control)   & $0.012$ & $.68$ & $0.016$ & $+0.012$ & ---     & $\Delta\approx 0$ \\
LOCATA azimuth           & $0.008$ & $.05$ & $0.007$ & $-0.013$ & $1.099$ & no detectable excess \\
Sperm-whale codas        & $0.046$ & $.98$ & $0.112$ & $+0.045$ & $1.010$ & no detectable excess \\
GOES protons             & $0.029$ & $.77$ & $0.048$ & $+0.030$ & $0.998$ & no detectable excess \\
\addlinespace
