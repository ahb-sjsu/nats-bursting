...
serves KV-cache pages (with more bits on keys, whose error the softmax amplifies) and even
supports attention computed directly on the compressed codes~\cite{onlinesoftmax,flashattention},
though that kernel-level work is out of scope here.
...
quantized keys and values are never reconstructed. Our paper does not propose a new codec; it
