...
pool, all same-node and warm, measuring per-task \emph{dispatch latency}: a tiny round-trip that
isolates fabric overhead (Table~\ref{tab:frameworks}). On \emph{throughput} every fabric is
ultimately GPU-bound---batched embedding on the warm pool scales to the two-GPU ceiling
(Fig.~\ref{fig:embed-scaling})---so we compare fabrics on dispatch overhead and capability, not peak
docs/s. (We deliberately do \emph{not} report a per-fabric fine-grained throughput: it is dominated
by each system's dispatch idiom and, for our request/reply arm, by the single-client driver rather
than the bus itself.)

nats-bursting's dispatch latency (1.1\,ms p50) is competitive: on par with
\texttt{ProcessPoolExecutor} (1.0\,ms), under Ray (2.3\,ms), and an order of magnitude below Dask
(13.9\,ms); its tail is looser (p99 ${\sim}52$\,ms, async/GC jitter). But raw dispatch is not the
contribution: Ray, Dask, Parsl and Globus Compute/funcX are mature task engines we do not try to
beat. What none of them carries is a \emph{host politeness budget}, an \emph{inbound-restricted WAN
...
\begin{table}[t]\centering\caption{Same-node warm-pool dispatch overhead on one GV100 (MiniLM
embedding, warm). ``disp.'' = tiny-task round-trip. Ray/Dask/ProcessPool/nats-bursting measured;
Parsl (HTEx) and funcX/Globus Compute characterized by design (funcX needs a hosted authenticated
endpoint). polite = host fair-use budget; WAN = inbound-restricted leaf federation; bus = burst is
an event-loop publish. Throughput is GPU-bound for all (Fig.~\ref{fig:embed-scaling}).}\label{tab:frameworks}
\small
\begin{tabular}{lrrccc}\toprule
fabric & disp.\ p50 & disp.\ p99 & polite & WAN & bus \\\midrule
\texttt{ProcessPool}   & 1.0\,ms  & 1.3\,ms  & no & no & no \\
Ray                    & 2.3\,ms  & 2.8\,ms  & no & no & no \\
Dask                   & 13.9\,ms & 90\,ms   & no & no & no \\
Parsl (HTEx)           & \multicolumn{2}{c}{by design} & no & no & no \\
funcX / Globus         & \multicolumn{2}{c}{by design} & no & part & no \\
\textbf{nats-bursting} & \textbf{1.1\,ms} & 52\,ms & \textbf{yes} & \textbf{yes} & \textbf{yes} \\
\bottomrule
\end{tabular}
\end{table}
