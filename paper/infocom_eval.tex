...
We evaluate on a testbed whose information-to-effect lag is measured rather than assumed. A home
...
questions organize the study, and each one defends a claim the theory makes.
	extbf{(Q1)} What is the lag $D$ of \eqref{eq:Ddef} that governs
Prop.~ef{prop:bound}, and how is it composed. \textbf{(Q2)} Does Algorithm~\ref{alg:adaptive} stay
...
\subsection{The lag and the violation bound (Q1)}
\label{sec:eval-detection}
The lag has two measured parts. A read-only \texttt{nvidia-smi} probe
...
per-GPU process census subject to the lag of Q1. Two profiles bracket the
...
measured lag $D{=}3$ epochs with $1.2\times10^{5}$ epochs per point.
...
The evaluation establishes three things. The lag is a measured
first-order term at about $2.5$\,s, dominated by worker start-up rather than by
polling, which is the composition \eqref{eq:Ddef} assumes. Under structured contention
