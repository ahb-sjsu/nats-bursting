...
class Worker:
    ...
    async def run(self) -> None:
        ...
        log.info(f"worker ready group={self.consumer_group} "
        ...
        while not stopping["flag"]:
            ...
            for msg in msgs:
                ...
                try:
                    ...
                    await nc.publish(
                        self.result_prefix + str(task_id),
                        json.dumps(result, default=str).encode(),
                    )
                    await msg.ack()
                    log.info(f"[{task_id}] {task_type} done in "
                             f"{result['duration_s']}s")
                except Exception as e:
                    log.exception(f"task loop: {e}")
                    try:
                        await msg.nak()
                    except Exception:
                        pass
